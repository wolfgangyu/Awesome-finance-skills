...
class StockTools:
    ...
    def search_ticker(self, query: str, limit: int = 5) -> List[Dict]:
        ...
        """
        clean_query = re.sub(r"\.(TW|US|T)$", "", query, flags=re.IGNORECASE)
...
def get_stock_analysis(ticker: str, db: DatabaseManager) -> str:
    ...
    report = [
        ...
        f"- **當前價**: {currency_symbol}{latest['close']:.2f}",
        f"- **時段漲跌**: {change:+.2f}%",
        f"- **最高/最低**: {currency_symbol}{df['high'].max():.2f} / {currency_symbol}{df['low'].min():.2f}",
        "\n### 最近交易概覽",
        "```",

...
class DatabaseManager:
    """
    AlphaEar 資料库管理器 - 负责存储热点資料、搜尋快取和股价資料
    使用 SQLite 进行持久化存储
    """
        ...
        for news in news_list:
            try:
                ...
                """, (
                    ...
                    news.get('url'),
                    news.get('content', ''),
                    news.get('publish_time'), # 新增支援发布時間
                    crawl_time,
    ...
    def get_search_cache(self, query_hash: str, ttl_seconds: Optional[int] = None) -> Optional[Dict]:
        """取得搜尋快取 (优先查 search_detail)"""
        cursor = self.conn.cursor()
        
        # 1. 尝试从 search_detail 取得展开的结构化資料
        cursor.execute("""
    ...
    def save_search_cache(self, query_hash: str, query: str, engine: str, results: Union[str, List[Dict]]):
        """保存搜尋结果 (同時保存到 search_cache 和 search_detail)"""
        cursor = self.conn.cursor()
    ...
    def get_stock_by_code(self, code: str) -> Optional[Dict[str, str]]:
        ...
        Args:
            code: 股票代號（美股如 "AAPL"、台股 4 位數字如 "2330"），可為字串或數字。

...
统一定义 ISQ 的各个维度、評分标准、和使用方法。
支援預設 template 和自定义 template。
"""
...
class ISQDimension(str, Enum):
    ...
    INTENSITY = "intensity"              # 强度/影響量级
    EXPECTATION_GAP = "expectation_gap"  # 预期差/市場认知差
    TIMELINESS = "timeliness"            # 時效性/窗口紧迫度
...
class ISQDimensionSpec(BaseModel):
    ...
    scale_factor: float = Field(default=1.0, description="显示時的缩放因子")
...
class ISQTemplate(BaseModel):
    ...
    # 評分指导
    scoring_guide: str = Field(..., description="評分指导说明")
    ...
    dimension_weights: Dict[str, float] = Field(default_factory=dict, description="维度權重")
...
class ISQScore(BaseModel):
    ...
    # 各维度評分
    scores: Dict[str, float] = Field(..., description="各维度評分")
    
    # 总分
    overall_score: float = Field(..., description="綜合評分")
    
    # 評分理由
    rationale: Dict[str, str] = Field(default_factory=dict, description="各维度評分理由")
...
DEFAULT_ISQ_TEMPLATE = ISQTemplate(
    ...
    template_name="标准投资訊號质量评估框架 (ISQ v1.0)",
    description="AlphaEar 預設的 ISQ 评估框架，用于标准化评估投资訊號的质量维度",
    
    dimensions={
        "sentiment": ISQDimensionSpec(
            name="情绪/走势",
            key="sentiment",
            description="基礎情绪偏向和市場走势判断",
        ...
        "expectation_gap": ISQDimensionSpec(
            name="预期差",
            key="expectation_gap",
            description="市場预期与现实之间的差距",
            range_type="0.0 到 1.0",
            scale_factor=1.0,
            examples={
                "0.0-0.2": "市場充分认知，预期差小",
                "0.2-0.5": "市場部分认知，存在一定预期差",
                "0.5-0.8": "市場认知不足，预期差较大，存在博弈空间",
                "0.8-1.0": "市場严重低估/高估，巨大预期差"
            },
        ...
        "timeliness": ISQDimensionSpec(
            name="時效性",
            key="timeliness",
            description="訊號的時間窗口紧迫度",
            range_type="0.0 到 1.0",
            scale_factor=1.0,
            examples={
                "0.0-0.2": "长期訊號，反应窗口 > 3 月",
                "0.2-0.5": "中期訊號，反应窗口 1-3 月",
                "0.5-0.8": "短期訊號，反应窗口 1 周 - 1 月",
                "0.8-1.0": "超短期訊號，反应窗口 < 1 周（需立即行动）"
            },
    ...
    scoring_guide="""
    ### ISQ 評分指导 (Investment Signal Quality)
    
    ISQ 框架用于多维度评估投资訊號的质量。每个訊號由 5 个维度组成：
    
    1. **情绪 (Sentiment)**: -1.0 到 1.0，表示看空(-)/中性(0)/看多(+)
    2. **确定性 (Confidence)**: 0.0 到 1.0，数值越高越确定
    3. **强度 (Intensity)**: 1 到 5，数值越高影響越大
    4. **预期差 (Expectation Gap)**: 0.0 到 1.0，市場预期与现实的差距
    5. **時效性 (Timeliness)**: 0.0 到 1.0，反应窗口的紧迫程度
    
    ### 綜合評分算法
    
    綜合評分 = 确定性 × 0.35 + 强度/5 × 0.30 + 预期差 × 0.20 + 時效性 × 0.15
    
    范围: 0.0 到 1.0
    - 0.0-0.3: 訊號质量较差，不建议跟进
    - 0.3-0.6: 訊號质量一般，可作参考
    - 0.6-0.8: 訊號质量良好，值得跟进
    - 0.8-1.0: 訊號质量优异，强烈推荐
...
使用以下 {len(template.dimensions)} 个维度评估訊號质量：
...
def load_templates_from_config(config_path: Optional[str] = None) -> None:
    """从配置目录載入所有 JSON 模板檔案，未找到则跳过，不影響預設模板。
    支援单个 JSON 檔案或目录（目录下的所有 .json 檔案）。
    """
    if config_path:
        path = Path(config_path)
    else:
        # 預設目录：config/isq_templates/
        # __file__ = src/schema/isq_template.py
        # parent = src/schema, parent.parent = src, parent.parent.parent = 專案根目录

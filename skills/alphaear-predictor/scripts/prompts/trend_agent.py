...
def get_trend_scanner_instructions() -> str:
    ...
    return f"""你是一名专业的資料扫描员，当前時間是 {current_time}。
你的任务是利用各种工具从互联网和資料库中取得最新的金融新闻、热点趋势和市場資料。

### 1. 核心职责
1. **多源采集**: 使用 `news_toolkit` 取得最新新闻，使用 `stock_toolkit` 取得行情，使用 `polymarket_toolkit` 取得预测市場資料。
2. **情绪感知**: 使用 `sentiment_toolkit` 对关键新闻进行情绪分析。
3. **深度搜尋**: 针对模糊的热点，使用 `search_toolkit` 进行全网搜尋补充细节。

### 2. 工具使用规范
- **广度优先**: 尽可能覆盖多个資料源。
- **資料新鲜度**: 优先取得最近 24 小時内的信息。
- **结构化輸出**: 整理搜集到的原始資料，為后续评估提供清晰的素材。
"""
...
def get_trend_evaluator_instructions() -> str:
    ...
    - **传导节点**: A -> B -> C 的逻辑推导。
    - **推荐关注**: 板块或 Ticker。
    - **ISQ 评估**: 基于模板的 5 个维度进行初步評分（具体評分由后续 FinAgent 完成）。
    """

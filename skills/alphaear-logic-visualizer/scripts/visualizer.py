...
class VisualizerTools:
    ...
    def generate_isq_radar_chart(sentiment: float, confidence: float, intensity: int, 
        ...
        schema = [
            ...
            opts.RadarIndicatorItem(name="影響力", max_=100),
            opts.RadarIndicatorItem(name="预期差", max_=100),
            opts.RadarIndicatorItem(name="時效性", max_=100),
        ]
    ...
    def render_drawio_to_html(xml_content: str, filename: str, title: str = "Logic Diagram") -> str:
        """
        將 Draw.io XML 渲染為套件含 Viewer 的 HTML 檔案
        """
        import json
        ...
        # 1. 转為 JSON 字串 (自動处理内部的引号转义、换行符转义等)
        json_str = json.dumps(config)
        
        # 2. 转為 HTML 属性安全的字串 (主要是转义单引号，因為我们在 HTML 中用单引号套件裹)
        import html

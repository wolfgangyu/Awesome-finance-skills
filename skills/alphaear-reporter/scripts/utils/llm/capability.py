...
def test_tool_call_support(model: Model) -> bool:
    """
    测试模型是否支援原生的 Tool Call (Function Calling)。
    通过尝试执行一个简单的加法工具来验证。
    """
    ...
    test_agent = Agent(
        ...
        instructions="请呼叫工具查询北京的天气，并直接回傳工具的輸出结果。"
    )
    ...
    try:
        ...
        # Agno 的 RunResponse 物件通常套件含 messages，我们可以檢查最后几条訊息
        has_tool_call = False
        ...
        if has_tool_call:
            ...
            # 如果没有 tool_calls 但回傳了正确答案，可能是模型通过纯文本模拟了工具呼叫（ReAct）
            # 或者根本没用工具。对于原生支援的判断，我们坚持要求有 tool_calls 结构。

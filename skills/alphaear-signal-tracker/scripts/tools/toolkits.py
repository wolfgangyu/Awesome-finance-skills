...
class StockToolkit(Toolkit):
    ...
    """
    股票工具套件 - 套件装 StockTools 為 Agno Toolkit
    ...
    def search_ticker(self, query: str) -> str:
        ...
        Args:
            query: 搜尋关键词，可以是股票代號（如 "600519"）或名称关键词（如 "茅台"、"宁德"、"比亚迪"）。
...
- 資料范围: {df.iloc[0]['date']} -> {latest['date']}

### 最近 15 个交易日详细資料 (OHLCV):
{history_str}
"""

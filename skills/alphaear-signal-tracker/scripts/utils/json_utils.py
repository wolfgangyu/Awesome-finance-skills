...
def extract_json(text: str) -> Optional[Any]:
    ...
    # e. 修复未终止的字串字面量问题：移除值中的实际换行符
    # LLM 可能在字串值中生成套件含真实 newline 的内容，导致 JSON 非法
    ...
    try:
        ...
        try:
            # 4. 尝试处理单引号问题 (JSON 规范要求双引号，但 LLM 常輸出单引号)
            # 这是一个简单的替换技巧，仅针对像 {'key': 'value'} 这样的结构
            # 注意：这可能会破坏套件含单引号的字串值，所以作為较后的回退
            ...
            try:
                # 5. 使用 ast.literal_eval 作為终极回退 (处理 Python 字典格式)
                # 提取第一个匹配的括号对内容
                # 寻找匹配的 { }
                stack = []

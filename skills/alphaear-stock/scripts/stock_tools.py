...
class StockTools:
    ...
    def get_stock_price(
        ...
        Returns:
            pd.DataFrame: 包含 date, open, close, high, low, volume, change_pct 欄位。
        """
        now = datetime.now()
        if not end_date:
    ...
    def get_stock_fundamentals(self, ticker: str) -> Dict:
        ...
        Returns:
            Dict: 包含 name, sector, industry, market_cap, pe_ratio, summary, currency 欄位。
        """
        market = detect_market(ticker)
        if market != "us":
            return {}
...
def get_stock_analysis(ticker: str, db: DatabaseManager) -> str:
    ...
    Returns:
        str: Markdown 格式的分析報告，包含價格走勢和關鍵指標。
    """
    tools = StockTools(db)
